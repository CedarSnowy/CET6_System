...
def search_exam_info(request, nid):
    """
    1. 打印所有可报名的考试信息
    2. 并在最下面添加报名按钮
...
def submit_enroll_info(request, nid):
    """
    1. 考试id, 身份证
    ...
    #         return render(request, "submit_enroll_info.html".format(nid), {"error": "重复报名"})
    #         # redirect("/homepage/{}/submit_enroll_info".format(nid), {"error": "重复报名"})
    ...
    #             return render(request, "submit_enroll_info.html".format(nid),
    #                           {"error": "身份证或考试号码错误,请重新确认"})
    #             # redirect("/homepage/{}/submit_enroll_info.html".format(nid), {"error": "身份证或考试号码错误, 请重新确认"})
    #         else:  # 如果均存在且没报过名
    ...
    #                                              payment_status="未支付")
    #             return render(request, "submit_enroll_info.html".format(nid), {"error": "报名成功，请及时缴费！"})
...
def receive_enroll_submit(request):
    ...
    if if_exist.exists():  # 如果已经报名了，直接提示
        ...
        queryset_for_exam = models.ExamInformation.objects.filter(exam_id=int(examid)).first()
        if queryset_for_id is None or queryset_for_exam is None:  # 如果user_id或exam_id不存在
            return render(request, "submit_enroll_info.html",
                          {"error": "身份证或考试号码错误,请重新确认"})
            # redirect("/homepage/{}/submit_enroll_info.html".format(nid), {"error": "身份证或考试号码错误, 请重新确认"})
        else:  # 如果均存在且没报过名
            models.Enrollment.objects.create(id_card_no=queryset_for_id, exam=queryset_for_exam,
